...
public:
    ...
    inline double getWinningRate();
    inline void setWinningRate(double winner,int color);
    inline int getWinner();
    inline void setWinner(int color);
private:
    int stateColor;
    double visitTime = 0;
    double wins = 0;
...
inline double MctsNode::getWinningRate() {
    return winningRate;
}


inline void MctsNode::setWinningRate(double winner, int color) {
    int Winner = (winner > 0.5)?Black:White;
    if(Winner == color)
    {
        winningRate = winner;
    }
    ...
    {
        winningRate = 1 - winner;
    }
}

inline void MctsNode::setWinner(int color) {
    winner = color;
}


inline int MctsNode::getWinner() {
    return winner;
}
...
inline void Bot::select(MctsNode*& workingNode)
{
    ...
    //workingNode = bestchild;
    //backpropagate(bestchild, bestchild->getWinner(), bestchild->getWinningRate());
...
inline void Bot::initPythonM()
{
    ...
    initPredict = PyObject_GetAttrString(myModule, "init_latest_model_predict");
...
{
    ofstream out("model/cnnInput1",ofstream::app);
...
inline void CNN::writeOutput(ChildZone cz, Bot& bot)
{
    ...
    double sum = 0;
    ofstream out("model/tempOut",ofstream::app);
    for (auto &r : cz)
    {
        coordinate p = r->getPrevAction();
        int x = p.first;
        int y = p.second;
        outBoard[x][y] = r->getVisit();
        sum += outBoard[x][y];
    }
    for (auto &r : cz)
    {
