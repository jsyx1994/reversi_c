...
void updateLast4(int i, int j, short newData)
{
...
public:
    ...
    inline bool ProcStep(MctsNode*& node,int xPos, int yPos, int color, bool checkOnly = false, bool usedInsimulate = true);
...
{
    ofstream out("/Users/mro/Desktop/cnnInput",ofstream::app);
...
inline void CNN::writeOutput(ChildZone cz, Bot& bot)
{
    ...
    double sum = 0;
    ofstream out("/Users/mro/Desktop/cnnOut",ofstream::app);
    for (auto &r : cz)
    {
  ...
  //  if(bot.gameEnded(cz[0]->getParent())) // 根状态游戏已结束
  //  {
  //    out << bot.judge(cz[0]->getParent()).first; // -1 , 1 , 0 ,BAI ,HEI , PING
  //  }
}
int main()//int argc , char **argv)
{
    ...
    //bot.outPutState(bot.getRootState());
    // 决策结束，输出结果（你只需修改以上部分）
